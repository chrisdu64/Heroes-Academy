...
export class HeroesDtoService {
  ...
  getHeroByforkjoin$(searchTerm: string): any {
    return forkJoin([this.getHeroesDtoByHeroDtoName$(searchTerm), this.getHeroesDtoByAbilityName$(searchTerm)]).pipe(
      tap(r => console.log("resultat:", r)),
      map(([searchByName, searchByAbility]) => ([...searchByName, ...searchByAbility].reduce((acc, cur) => {
      ...
      map(searchResult => searchResult.sort((a, b) => (a.name > b.name ? 1 : (a.name === b.name ? 0 : -1)))),
      tap(r => console.log("res 2:", r)),
